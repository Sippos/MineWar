...
func _run_line_wars_handoff() -> void:
	...
	# Reproduce the real gate input: Up is still held when the LineWars transition
	# begins. The new front must wait for neutral input instead of carrying this
	# movement onto the board.
	Input.action_press("p1_up")
	await preparation.call("_begin_mode", GameMode.Mode.LINE_WARS, "", "Testing LineWars handoff")
	await _wait_frames(3)

	var board_center := maze.to_global(Vector2.ZERO)
	print("LINEWARS_HANDOFF_POSITION builder=", peon.global_position, " center=", board_center, " distance=", peon.global_position.distance_to(board_center))
	...
	_expect(peon.global_position.distance_to(board_center) < 1.0, "LineWars should place the builder at the board center")
	await get_tree().create_timer(0.2).timeout
	print("LINEWARS_HELD_POSITION builder=", peon.global_position, " center=", board_center, " distance=", peon.global_position.distance_to(board_center))
	_expect(peon.global_position.distance_to(board_center) < 1.0, "Held gate input should not move the builder after the handoff")
	...
	_expect(peon.global_position.x < board_center.x - 2.0, "Builder movement should resume after one neutral input frame")
