...
func _try_attach(node: Node) -> void:
	...
	var keeper := node.get_node("SinglePlayerButton") as Button
	keeper.text = "MineWars Keeper"
	keeper.tooltip_text = "Mine under pressure, build an RPG hero, and defend the base directly."
