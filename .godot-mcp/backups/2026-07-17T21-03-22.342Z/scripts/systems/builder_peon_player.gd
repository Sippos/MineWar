...
func set_controlled(value: bool) -> void:
	...
	if camera:
		...
		if value:
			# Front switching and the overworld-to-LineWars handoff can teleport the
			# peon a long distance. Reset smoothing so the board is framed correctly
			# on the first rendered frame instead of sliding in from the old gate.
...
func _physics_process(delta: float) -> void:
	...
	# A direction is normally still held while crossing an overworld mode gate.
	# Do not carry that movement into the newly activated front. Requiring one
	# neutral frame keeps the peon centered on the LineWars board and prevents
	# accidental movement whenever control switches between hero and peon.
