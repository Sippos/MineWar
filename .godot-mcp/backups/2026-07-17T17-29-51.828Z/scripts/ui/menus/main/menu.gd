...
func _ready() -> void:
	...
	$MazeModeButton.pressed.connect(_on_maze_mode_pressed)
...
func _layout_for_screen(screen_size_override: Vector2 = Vector2.ZERO) -> void:
	...
	_layout_button($MazeModeButton, center_x, buttons_top + (button_h + gap), button_w, button_h)
	_layout_button($VSModeButton, center_x, buttons_top + (button_h + gap) * 2.0, button_w, button_h)
	_layout_button($VSOnlineButton, center_x, buttons_top + (button_h + gap) * 3.0, button_w, button_h)
	_layout_button($ControlsButton, center_x, buttons_top + (button_h + gap) * 4.0, button_w, button_h)
...
func _main_button_layout(screen_size: Vector2) -> Dictionary:
	...
	var title_clearance: float = 18.0 if compact else 25.0
	...
	var stack_height: float = button_height * 5.0 + gap * 4.0
...
func _on_maze_mode_pressed() -> void:
	get_tree().change_scene_to_file("res://maze_vs_prototype.tscn")
