...
func _setup() -> void:
	...
	# The existing world process only owns the old random-wave timer in single
	# player. Mining, combat, pickups, abilities, and the base keep processing on
	# their own nodes while this controller owns the new invasion rhythm.
	...
	reset_button.pressed.connect(_on_reset_maze_pressed)
...
func _process(delta: float) -> void:
	...
	if not run_started:
		# The existing preparation controller still performs hero/base selection.
		# Mirroring the hidden hero to the peon lets its proximity logic operate on
		# the peon without replacing or duplicating that mature menu system.
...
func _unhandled_input(event: InputEvent) -> void:
	if run_started and event.is_action_pressed("switch_front"):
		_toggle_front()
...
func _release_portal_queue() -> void:
	var released := portal_queue.duplicate()
	portal_queue.clear()
	portal_charge_remaining = 0.0
	for index in released.size():
		_spawn_arena_enemy(released[index], index)
...
func _spawn_arena_enemy(enemy_type: String, index: int) -> void:
	var enemy := ENEMY_SCENE.instantiate()
	world.add_child(enemy)
	var angle := TAU * float(index) / float(maxi(portal_queue.size() + 1, 4))
