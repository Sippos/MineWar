...
func _try_attach(node: Node) -> void:
	if not is_instance_valid(node) or not str(node.name).begins_with("Player") or not (node is CharacterBody2D):
		return
	if node.get_node_or_null("HeroRPGController") == null:
		var rpg_controller := Node.new()
		rpg_controller.name = "HeroRPGController"
		rpg_controller.set_script(HERO_RPG_SCRIPT)
		node.add_child(rpg_controller)
