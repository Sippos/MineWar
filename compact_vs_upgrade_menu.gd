...
func _add(label_text: String, method_name: String, icon_path: String, cost: String, disabled: bool = false) -> void:
	var button := _button(label_text, cost, method_name, icon_path, disabled)
...
func _button(title_text: String, cost_text: String, method_name: String, icon_path: String = "", disabled: bool = false) -> Button:
	...
	heading.add_theme_color_override("font_color", Color(0.58, 0.55, 0.5) if disabled else Color(1.0, 0.9, 0.7))
	heading.mouse_filter = Control.MOUSE_FILTER_IGNORE
	labels.add_child(heading)
	if cost_text != "":
		var cost := Label.new()
		cost.text = "OWNED" if disabled else cost_text
		cost.add_theme_font_size_override("font_size", 12)
		cost.add_theme_color_override("font_color", Color(0.48, 0.68, 0.48) if disabled else Color(1.0, 0.75, 0.28))
