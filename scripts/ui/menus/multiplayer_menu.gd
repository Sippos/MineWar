...
func _open_local_hero_selection(mode_name: String) -> void:
...
func _open_online_hero_selection() -> void:
	...
	selector.setup(2, "online_exploration_vs")
	add_child(selector)
	selector.tree_exited.connect(_restore_focus.bind(legacy_online_button))

func _on_coop_pressed() -> void:
	GameMode.set_mode(GameMode.Mode.EXPLORATION)
	_open_local_hero_selection("local_coop")

func _on_maze_pressed() -> void:
	GameMode.set_mode(GameMode.Mode.LINE_WARS)
	_open_local_hero_selection("maze_vs")
