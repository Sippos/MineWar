...
const CLICK_DEBOUNCE_MSEC := 180
const SURFACE_BOARD_RECT := Rect2(-408.0, -180.0, 816.0, 360.0)
...
var last_switch_request_msec := -10000
var last_reset_request_msec := -10000
var mine_canvas_color := Color.WHITE
var original_base_z_index := 0
...
func _setup() -> void:
	...
	existing_hud = world.get_node_or_null("HUD") as CanvasLayer
	canvas_modulate = world.get_node_or_null("CanvasModulate") as CanvasModulate
	if hero == null or base == null:
		push_error("Dual front controller requires the normal Player and Base nodes")
		return
	if canvas_modulate:
		mine_canvas_color = canvas_modulate.color
	original_base_z_index = base.z_index

	_ensure_switch_action()
	# The old world process owns the former random-wave timer. Mining, combat,
	# pickups, abilities, and the base are separate nodes and remain available
	# while this controller owns the surface maze and portal rhythm.
	...
	switch_button.pressed.connect(_request_toggle_front)
	reset_button.pressed.connect(_request_reset_maze)
	dual_front_hud.visible = false

	_set_hero_preparation_state()
	builder_peon.call("set_controlled", true)

func release_to_standard_run() -> void:
	# MineWars and Adventure use the real hero directly. Preserve the exact gate
	# position, restore the mine presentation, and remove only the temporary
	# surface avatar/controller pieces.
	if hero and is_instance_valid(hero):
		if builder_peon and is_instance_valid(builder_peon):
			hero.global_position = builder_peon.global_position
		hero.visible = true
	...
	if existing_hud and is_instance_valid(existing_hud):
		existing_hud.visible = true
	if base and is_instance_valid(base):
		base.visible = true
		base.z_index = original_base_z_index
	if canvas_modulate and is_instance_valid(canvas_modulate):
		canvas_modulate.color = mine_canvas_color
	if world and is_instance_valid(world):
		world.set_process(true)
		if world.has_meta("dual_front_active"):
			world.remove_meta("dual_front_active")
	if builder_peon and is_instance_valid(builder_peon):
		builder_peon.queue_free()
	if surface_maze and is_instance_valid(surface_maze):
		surface_maze.queue_free()
	queue_free()
...
func _input(event: InputEvent) -> void:
	...
	if not is_pressed:
		return
	if _control_contains_viewport_point(switch_button, click_position):
		_request_toggle_front()
		get_viewport().set_input_as_handled()
	elif reset_button.visible and _control_contains_viewport_point(reset_button, click_position):
		_request_reset_maze()
		get_viewport().set_input_as_handled()

func _control_contains_viewport_point(control: Control, viewport_point: Vector2) -> bool:
...
func _process(delta: float) -> void:
	...
	if not run_started:
		# The preparation controller handles hero/base selection. Mirroring the
		# hidden hero lets it use the visible peon's position until a mode is chosen.
...
func _begin_dual_front_run() -> void:
	run_started = true
	# The previous handoff left the peon at the northern overworld gate while the
	# board lived hundreds of pixels away. Center the builder on the actual board
	# before enabling its camera so LineWars opens fully visible and playable.
	builder_peon.global_position = surface_maze.to_global(Vector2.ZERO)
	builder_peon.set("movement_bounds", Rect2(surface_maze.to_global(SURFACE_BOARD_RECT.position), SURFACE_BOARD_RECT.size))
...
func _request_toggle_front() -> void:
	var now := Time.get_ticks_msec()
	if now - last_switch_request_msec < CLICK_DEBOUNCE_MSEC:
		return
	last_switch_request_msec = now
	_toggle_front()

func _request_reset_maze() -> void:
	var now := Time.get_ticks_msec()
	if now - last_reset_request_msec < CLICK_DEBOUNCE_MSEC:
		return
	last_reset_request_msec = now
...
func _apply_front_control() -> void:
	...
	builder_peon.visible = peon_is_active
	surface_maze.call("set_builder_active", peon_is_active)
	surface_maze.visible = peon_is_active

	hero.process_mode = Node.PROCESS_MODE_DISABLED if peon_is_active else Node.PROCESS_MODE_INHERIT
	hero.velocity = Vector2.ZERO
	hero.visible = not peon_is_active
	var hero_camera := hero.get_node_or_null("Camera2D") as Camera2D
	if hero_camera:
		hero_camera.enabled = not peon_is_active

	# The mine HUD and dark mine tint made the surface board look like a broken
	# overlay. The peon front now has a clean presentation; switching to the hero
	# restores the mine exactly.
	if existing_hud and is_instance_valid(existing_hud):
		existing_hud.visible = not peon_is_active
	if base and is_instance_valid(base):
		base.visible = not peon_is_active
	if canvas_modulate and is_instance_valid(canvas_modulate):
		canvas_modulate.color = Color.WHITE if peon_is_active else mine_canvas_color
...
func _release_portal_queue() -> void:
	...
	if hud and hud.has_method("show_notice"):
		hud.show_notice("BREACH! %d enemies emerged beside the base." % released.size(), 3.2)
