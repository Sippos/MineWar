...
func _activate() -> void:
	...
	if world == null or not is_instance_valid(world):
		queue_free()
		return
	if bool(world.get("is_vs_mode")) or not GameMode.is_siege():
		queue_free()
		return
	...
	if block_layer == null or player == null:
		push_error("Siege Mode requires BlockLayer and Player.")
		queue_free()
		return
	...
	if hud and hud.has_method("show_notice"):
		hud.show_notice("MINEWARS: mine, carry resources home, build your hero, and return before the assault.", 6.0)
...
func _create_entrance_marker() -> void:
	...
	entrance_marker.name = "SiegeSurfaceGate"
