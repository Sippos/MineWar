...
func _refresh_cell(cell: Vector2i) -> void:
	...
	sprite.offset = Vector2(0, -(TILE_SIZE + depth))
	...
	sprite.position = to_local(top_left_global) + Vector2(0, TILE_SIZE + depth)
	sprite.visible = true
...
func _build_extrusion_image(source_id: int, mask: int, round_left: bool = false, round_right: bool = false) -> Image:
	...
	# The front face owns a separate bottom-corner mask. Horizontal runs remain
