...
const KEEPER_SCRIPT := preload("res://scripts/systems/world_generation/keeper_mode_controller.gd")
...
func _try_attach(node: Node) -> void:
	...
	if not node.has_node("BlockLayer") or not node.has_node("Base") or not node.has_node("Player"):
		return
	if node.get("is_vs_mode") == true or node.has_node("KeeperModeController"):
		return
	var controller := Node2D.new()
	controller.name = "KeeperModeController"
	controller.set_script(KEEPER_SCRIPT)
