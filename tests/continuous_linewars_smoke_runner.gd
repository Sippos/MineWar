...
func _run_smoke_test() -> void:
	...
	# The controller must not teleport either avatar. Check before the newly
	# enabled hero physics performs any normal collision settling.
	_expect(hero.global_position == hero_position and peon.global_position == peon_position, "Switching should preserve both physical positions")
	await _wait_frames(2)
