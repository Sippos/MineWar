...
const OUTPUT_PATH: String = "res://Hero_Showdown_Background.png"
...
func _ready() -> void:
	...
	var save_error: Error = canvas.save_png(output_absolute)
	if save_error != OK:
...
func _build_background(img: Image) -> void:
	...
	for y: int in range(0, HEIGHT, 8):
		...
		var top: Color = Color8(7, 10, 22)
		var middle: Color = Color8(16, 25, 39)
		var bottom: Color = Color8(30, 22, 31)
		var color: Color = top.lerp(middle, minf(t * 1.65, 1.0)) if t < 0.61 else middle.lerp(bottom, (t - 0.61) / 0.39)
		img.fill_rect(Rect2i(0, y, WIDTH, 8), color)

	# Distant masonry silhouettes.
	for y: int in range(104, 520, 48):
		var offset: int = 0 if int(y / 48) % 2 == 0 else 24
		for x: int in range(-offset, WIDTH, 72):
			var shade: Color = Color8(19 + rng.randi_range(0, 8), 29 + rng.randi_range(0, 8), 43 + rng.randi_range(0, 9), 150)
	...
	for x: int in range(0, WIDTH, 32):
		var depth: int = rng.randi_range(24, 116)
		img.fill_rect(Rect2i(x, 32, 32, depth), Color8(12, 13, 22))
		img.fill_rect(Rect2i(x + 5, 46, 7, maxi(4, depth - 20)), Color8(24, 26, 38, 180))
		if x % 96 == 0:
			var spike_h: int = rng.randi_range(34, 104)
			_draw_pixel_triangle(img, Vector2i(x + 16, 54 + depth), spike_h, 18, Color8(9, 10, 17), true)

	# Heavy side walls frame the confrontation.
	for side: int in [0, 1]:
		var base_x: int = 0 if side == 0 else WIDTH - 104
		img.fill_rect(Rect2i(base_x, 62, 104, 520), Color8(12, 15, 24))
		for y: int in range(80, 560, 42):
			var inset: int = rng.randi_range(8, 24)
			var bx: int = base_x + inset if side == 0 else base_x + 104 - inset - 58
	...
	for y: int in range(576, HEIGHT, 42):
		var offset: int = 0 if int(y / 42) % 2 == 0 else 36
		for x: int in range(-offset, WIDTH, 94):
			var shade_value: int = 29 + rng.randi_range(0, 13)
			img.fill_rect(Rect2i(x + 3, y + 3, 86, 34), Color8(shade_value, shade_value - 3, shade_value + 5))
			img.fill_rect(Rect2i(x + 9, y + 8, 45, 4), Color8(72, 64, 72, 105))

	for i: int in range(45):
		var px: int = rng.randi_range(18, WIDTH - 20)
		var py: int = rng.randi_range(578, HEIGHT - 14)
		var chunk_size: int = rng.randi_range(3, 10)
		img.fill_rect(Rect2i(px, py, chunk_size, maxi(2, int(chunk_size / 2))), Color8(67, 57, 66, rng.randi_range(80, 175)))


func _build_centerpiece(img: Image) -> void:
	var center: Vector2i = Vector2i(WIDTH / 2, 345)

	# Two opposing pools of colored atmosphere.
	for radius: int in range(190, 30, -16):
		var alpha: int = int(3 + (190 - radius) * 0.08)
	...
	for radius: int in range(116, 20, -12):
	...
	for i: int in range(34):
		var py: int = rng.randi_range(205, 520)
		var left_x: int = rng.randi_range(500, 620)
		var right_x: int = rng.randi_range(660, 780)
		var spark_size: int = rng.randi_range(2, 6)
		img.fill_rect(Rect2i(left_x, py, spark_size * 2, spark_size), Color8(255, 157, 72, rng.randi_range(100, 235)))
		img.fill_rect(Rect2i(right_x, py, spark_size * 2, spark_size), Color8(87, 205, 255, rng.randi_range(100, 235)))
...
func _place_hero(canvas: Image, path: String, row: int, column: int, cell_w: int, cell_h: int, anchor: Vector2i, target_height: int, glow_color: Color) -> void:
	...
	var frame_rect: Rect2i = Rect2i(column * cell_w, row * cell_h, cell_w, cell_h)
	var frame: Image = sheet.get_region(frame_rect)
	frame = _crop_transparent(frame)
	if frame.is_empty():
		push_error("Hero frame was empty: %s row %d column %d" % [path, row, column])
		return

	var target_width: int = maxi(1, int(round(float(frame.get_width()) * float(target_height) / float(frame.get_height()))))
	frame.resize(target_width, target_height, Image.INTERPOLATE_NEAREST)

	var position: Vector2i = Vector2i(anchor.x - int(frame.get_width() / 2), anchor.y - frame.get_height())
	_draw_shadow(canvas, Vector2i(anchor.x, anchor.y - 5), maxi(44, int(frame.get_width() / 2)), Color8(0, 0, 0, 155))

	var dark_outline: Image = _alpha_tint(frame, Color8(4, 5, 9, 235))
	var outline_offsets: Array[Vector2i] = [Vector2i(-4, 0), Vector2i(4, 0), Vector2i(0, -4), Vector2i(0, 4), Vector2i(-3, -3), Vector2i(3, -3), Vector2i(-3, 3), Vector2i(3, 3)]
	...
	var glow: Image = _alpha_tint(frame, glow_color)
	var glow_offsets: Array[Vector2i] = [Vector2i(-9, 0), Vector2i(9, 0), Vector2i(0, -9), Vector2i(0, 9), Vector2i(-7, -7), Vector2i(7, -7), Vector2i(-7, 7), Vector2i(7, 7)]
	for offset: Vector2i in glow_offsets:
...
func _crop_transparent(source: Image) -> Image:
	var min_x: int = source.get_width()
	var min_y: int = source.get_height()
	var max_x: int = -1
	var max_y: int = -1
	...
	var margin: int = 2
	min_x = maxi(0, min_x - margin)
	min_y = maxi(0, min_y - margin)
	max_x = mini(source.get_width() - 1, max_x + margin)
	max_y = mini(source.get_height() - 1, max_y + margin)
...
func _draw_shadow(img: Image, center: Vector2i, radius: int, color: Color) -> void:
	var half_height: int = maxi(7, int(radius / 5))
	for dy: int in range(-half_height, half_height + 1, 3):
		var normalized: float = absf(float(dy)) / float(half_height)
		var half_width: int = int(radius * (1.0 - normalized * normalized))
...
func _draw_diamond(img: Image, center: Vector2i, radius: int, color: Color) -> void:
	for dy: int in range(-radius, radius + 1, 4):
		var half_width: int = radius - absi(dy)
		if half_width > 0:
...
func _draw_pixel_triangle(img: Image, tip: Vector2i, height: int, half_width: int, color: Color, points_down: bool) -> void:
	for step: int in range(0, height, 4):
		var progress: float = float(step) / float(height)
		var width_here: int = int(half_width * progress)
		var y: int = tip.y - step if points_down else tip.y + step
		img.fill_rect(Rect2i(tip.x - width_here, y, maxi(2, width_here * 2), 4), color)


func _draw_torch(img: Image, center: Vector2i, core: Color, edge: Color) -> void:
	# Stepped light halo.
	for radius: int in range(82, 12, -10):
...
func _add_foreground(img: Image) -> void:
	...
	for i: int in range(18):
		var x: int = rng.randi_range(20, WIDTH - 100)
		var y: int = rng.randi_range(612, 704)
		var strip_width: int = rng.randi_range(35, 120)
		img.fill_rect(Rect2i(x, y, strip_width, rng.randi_range(2, 5)), Color8(130, 119, 133, rng.randi_range(18, 50)))
