...
const FONT_URL := "https://raw.githubusercontent.com/google/fonts/main/ofl/grenzegotisch/GrenzeGotisch%5Bwght%5D.ttf"
const LICENSE_URL := "https://raw.githubusercontent.com/google/fonts/main/ofl/grenzegotisch/OFL.txt"

func _ready() -> void:
	var target_dir := ProjectSettings.globalize_path("res://assets/fonts/grenze_gotisch")
	...
	var font_path := target_dir.path_join("GrenzeGotisch-Variable.ttf")
	...
	if font_code == 0 and license_code == 0 and FileAccess.file_exists(font_path) and FileAccess.file_exists(license_path):
		print("MINEWARS_FONT_DOWNLOAD_OK ", font_path)
		get_tree().quit(0)
	else:
		push_error("Font download failed: font=%d license=%d" % [font_code, license_code])
		get_tree().quit(1)
