...
func _ready() -> void:
	...
	for x in range(5 * 32, 6 * 32):
		if image.get_pixel(x, top_y).a <= 0.95:
			push_error("Preview top row lost full width")
			get_tree().quit(1)
			return
	var bottom_y := top_y + 31
	if image.get_pixel(5 * 32, bottom_y).a > 0.05 or image.get_pixel(6 * 32 - 1, bottom_y).a > 0.05:
		push_error("Preview bottom outer corners are not rounded")
		get_tree().quit(1)
		return
	if image.get_pixel(5 * 32 + 16, bottom_y).a <= 0.95:
		push_error("Preview bottom center was incorrectly removed")
		get_tree().quit(1)
		return
	...
	var runtime_image := renderer.call("_build_extrusion_image", 1, 4 | 8 | 2) as Image
	for x in range(64):
		if runtime_image.get_pixel(x, 64).a <= 0.95:
			push_error("Runtime top row lost full width")
			get_tree().quit(1)
			return
	if runtime_image.get_pixel(0, 95).a > 0.05 or runtime_image.get_pixel(63, 95).a > 0.05:
		push_error("Runtime bottom outer corners are not rounded")
		get_tree().quit(1)
		return
	if runtime_image.get_pixel(32, 95).a <= 0.95:
		push_error("Runtime bottom center was incorrectly removed")
		get_tree().quit(1)
		return
	print("PASS: front faces stay full-width at top and round only at lower outer corners")
	get_tree().quit(0)
