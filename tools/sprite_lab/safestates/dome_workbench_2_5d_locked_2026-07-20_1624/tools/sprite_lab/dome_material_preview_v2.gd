...
const MATERIAL_TIERS: Array[String] = ["unmineable", "easy", "medium", "hard"]
...
func _rebuild_material_textures() -> void:
	...
	for tier in MATERIAL_TIERS:
		if not border_images.has(tier) or not convex_images.has(tier) or not corner_images.has(tier):
			continue
		var border := border_images[tier] as Image
		var joint := convex_images[tier] as Image
		var corner := corner_images[tier] as Image
...
func _build_silhouette_data() -> Dictionary:
	...
	for y in range(MAP_SIZE.y):
		for x in range(MAP_SIZE.x):
			...
			if mask < images.size():
				_write_mask_image(images[mask] as Image, cell * CELL_SIZE, cell_type, width, height, solid, owners)
	...
	for y in range(MAP_SIZE.y):
		for x in range(MAP_SIZE.x):
			...
			for rule_value: Variant in rules:
				...
				_write_mask_image(hole_images[frame] as Image, Vector2i(roundi(patch_rect.position.x), roundi(patch_rect.position.y)), owner_type, width, height, solid, owners)

	return {"width": width, "height": height, "solid": solid, "owners": owners}
