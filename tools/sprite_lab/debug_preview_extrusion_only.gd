...
func _ready() -> void:
	...
	image.save_png("res://tools/sprite_lab/debug_front_masks/preview_extrusion_only_32.png")
	image.resize(image.get_width() * 3, image.get_height() * 3, Image.INTERPOLATE_NEAREST)
	image.save_png("res://tools/sprite_lab/debug_front_masks/preview_extrusion_only_32_scaled.png")
	get_tree().quit(0)
