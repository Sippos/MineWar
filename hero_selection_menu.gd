...
func _ready() -> void:
	...
	_layout_for_screen()
	update_ui(1)
	if current_mode == Mode.VS_LOCAL: update_ui(2)
	start_btn.grab_focus()

func _layout_for_screen() -> void:
	var screen_size = get_viewport().get_visible_rect().size
	if screen_size.x <= 0.0 or screen_size.y <= 0.0: return
	...
	start_btn.custom_minimum_size = button_size; back_btn.custom_minimum_size = button_size
...
func _configure_sprite_container(container: Control, sprite: Sprite2D, sprite_size: float) -> void:
	...
	sprite.scale = Vector2.ONE * (sprite_size / 128.0)

func change_hero(player_id: int, dir: int) -> void:
	if player_id == 1:
		p1_index = (p1_index + dir + available_heroes.size()) % available_heroes.size(); update_ui(1)
	else:
		p2_index = (p2_index + dir + available_heroes.size()) % available_heroes.size(); update_ui(2)

func _is_hero_playable(_hero_name: String) -> bool:
	return true # Playtest mode: progression locking can be restored later.
...
func update_ui(player_id: int) -> void:
	...
	if player_id == 1:
		p1_label.text = h_name + " (Playtest)"; p1_sprite.texture = tex; p1_sprite.modulate = Color.WHITE
	else:
		p2_label.text = h_name + " (Playtest)"; p2_sprite.texture = tex; p2_sprite.modulate = Color.WHITE
	start_btn.disabled = false

func _on_start_pressed() -> void:
	Global.hero_p1 = available_heroes[p1_index]
	Global.current_hero = Global.hero_p1
	Global.hero_p2 = available_heroes[p2_index] if current_mode == Mode.VS_LOCAL else Global.hero_p1
	if current_mode == Mode.SINGLE_PLAYER: get_tree().change_scene_to_file("res://main.tscn")
	elif current_mode == Mode.VS_LOCAL: get_tree().change_scene_to_file("res://vs_mode.tscn")
	else: get_tree().change_scene_to_file("res://online_lobby.tscn")
