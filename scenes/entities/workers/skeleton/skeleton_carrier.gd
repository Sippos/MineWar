...
func _ready():
	add_to_group("skeleton_carriers")
...
func _physics_process(delta):
	...
	if state == "IDLE":
		...
		if global_position.distance_to(target_gem.global_position) < 24.0:
			...
			if base_node and base_node.has_signal("gems_deposited"):
		...
		# Skeletons don't return, but just in case they get stuck in this state
		queue_free()
...
func _spawn_death_particles() -> void:
	...
	burst.spread = 60
	burst.initial_velocity_min = 40
	burst.initial_velocity_max = 90
	burst.scale_amount_min = 2.0
	burst.scale_amount_max = 4.0
	burst.color = Color(0.7, 0.2, 0.8) # Undead magic color
	burst.amount = 15
